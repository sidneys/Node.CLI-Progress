...
module.exports = class Bar{
    ...
    render(){
        ...
        if (this.lastDrawnString != s){
            ...
            // prepare 2 lines for writing s(tring) and optional annotation
            this.terminal.write(os.EOL + os.EOL);
            this.terminal.cursorTo(0, null);
            this.terminal.moveCursor(0, -2);

            // write output
            this.terminal.write(s);

            if (this.annotation !== null){
                // left-align annotation with s(tring)
                const sLeftMargin = s.match(/^\s*/)[0].length;

                // right-trim & ellipse annotation
                const sWidth = _stringWidth(s);
                this.annotation = ' '.repeat(sLeftMargin) + this.annotation.substring(0, sWidth - sLeftMargin);
                this.annotation += (this.annotation.length > sWidth - sLeftMargin) ? '…' : '';

                // move cursor below s(tring)
                this.terminal.moveCursor(null, 1);

                // move cursor to start of line
                this.terminal.cursorTo(0, null);

                // write annotation
                this.terminal.write(this.annotation);
    ...
    start(total, startValue, payload, annotation){
        // set initial values
    ...
    update(current, payload, annotation){
        // update value
        this.value = current;
    ...
    // update the bar value
    increment(step, payload, annotation){
        step = step || 1;
        this.update(this.value + step, payload, annotation);
    }
